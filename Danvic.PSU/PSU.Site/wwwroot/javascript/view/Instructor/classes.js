...
$.dataTableSetting = {
    ...
    ajax: function (data, callback, settings) {
        ...
        param.SName = $('#name').val();//班级名称
        param.SMajor = $('#major').val();//专业姓名
        param.SQQ = $('#qq').val();//班级QQ

        //ajax请求数据
        $.ajax({
            type: "POST",
            url: "/Instructor/User/SearchClass",
            cache: false,  //禁用缓存
            data: {
...
$(function () {
    ...
    var dataTable = $('#classes-table').dataTable($.dataTableSetting);
    ...
    $(document).on('click',
        ...
        function () {
            ...
            dataTable = $('#classes-table').dataTable($.dataTableSetting);
        });
    ...
    $(document).on('click',
        '#edit',
        function () {
            window.location.href = '/Instructor/User/EditClass/' + $(this).attr('data-id');
        });
})