...
$.dataTableSetting = {
    ...
    ajax: function (data, callback, settings) {
        ...
        $.ajax({
            type: "POST",
            url: "/Administrator/Basic/SearchStudent",
            cache: false,  //禁用缓存
            data: {
...
$(function() {
    var dataTable = $('#student-table').dataTable($.dataTableSetting);
    ...
    $(document).on('click',
        ...
        function() {
            ...
            dataTable = $('#student-table').dataTable($.dataTableSetting);
        });
    ...
    $(document).on('click',
        '#edit',
        function() {
            window.location.href = '/Administrator/Basic/EditStudent/' + $(this).attr('data-id');
        });
    ...
    $(document).on('click',
        ...
        function() {
            ...
            window.bootbox.confirm({
                message: '学生学号：<b class="text-red">' + id + '</b>，确定删除该条学生数据吗？',
                buttons: {
                    confirm: {
                        label: '确定',
                ...
                callback: function(result) {
                    if (result) {
                        $.ajax({
                            url: '/Administrator/Basic/DeleteStudent',
                            type: 'POST',
                            dataType: 'Json',
                            data: {
                                id: id
                            ...
                            success: function(result) {
                                window.bootbox.alert({
                                    ...
                                    callback: function() {
                                        window.location = "/Administrator/Basic/Student";
                                    }
                                });
                            },
                            error: function(msg) {
