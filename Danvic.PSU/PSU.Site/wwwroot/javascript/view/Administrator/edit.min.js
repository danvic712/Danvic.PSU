 ...
 *   Author: Danvic712
 */
$(function(){var n=new Simditor({textarea:$("#editor"),toolbar:["title","bold","italic","fontScale","color","ol","ul","table","link","hr","indent","outdent","alignment",]});$(document).on("click","#save",function(){if($("#title").val().length===0)return bootbox.dialog({message:"公告标题不能为空"}),!1;if($("#target").val()==="0")return bootbox.dialog({message:"请选择针对用户"}),!1;if($("#type").val()==="0")return bootbox.dialog({message:"请选择公告类型"}),!1;if($("#editor").val().length===0)return bootbox.dialog({message:"公告内容不能为空"}),!1;if($("#editor").val().length>1e3)return bootbox.dialog({message:"公告内容不能超过1000个字符"}),!1;var n={};n.Id=$(this).attr("data-id");n.Title=$("#title").val();n.Target=$("#target").val();n.Type=$("#type").val();n.Content=$("#editor").val();$(this).attr("disabled","disabled");$.ajax({type:"POST",url:"/Administrator/Home/Edit",data:n,dataType:"json",success:function(n){n.msg!==undefined&&(bootbox.dialog({message:n.msg}),n.success&&setTimeout(function(){window.location.href="/Administrator/Home/Bulletin"},2e3));$("#save").removeAttr("disabled")},error:function(n){console.log(n);$("#save").removeAttr("disabled")}})})});