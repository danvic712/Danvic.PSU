 ...
 *   Author: Danvic712
 */
$(function(){var n=new Simditor({textarea:$("#editor")});$(document).on("click","#save",function(){if($("#title").val().length===0){bootbox.alert("公告标题不能为空");return}if($("#title").val().length>20){bootbox.alert("公告标题不能超过20个字符");return}if($("#target").val()==="0"){bootbox.alert("请选择针对用户");return}if($("#type").val()==="0"){bootbox.alert("请选择公告类型");return}if($("#editor").val().length===0){bootbox.alert("请输入公告内容");return}if($("#editor").val().length>1e3){bootbox.alert("公告内容不能超过1000个字符");return}var n={};n.Id=$(this).attr("data-id");n.Title=$("#title").val();n.Target=$("#target").val();n.Type=$("#type").val();n.Content=$("#editor").val();$(this).attr("disabled","disabled");$.ajax({type:"POST",url:"/Administrator/Home/Edit",data:n,dataType:"json",success:function(n){n.msg!==undefined&&(bootbox.dialog({message:n.msg,closeButton:!1}),n.success?setTimeout(function(){window.location.href="/Administrator/Home/Bulletin"},2e3):setTimeout(function(){window.location.href=window.location.href},2e3));$("#save").removeAttr("disabled")},error:function(n){console.log(n);$("#save").removeAttr("disabled")}})})});