 ...
 *   Author: Danvic712
 */
$(function(){var n=new Simditor({textarea:$("#editor")});$("#department_form").validate();$(document).on("click","#save",function(){if($("#name").val().length===0){bootbox.alert("部门/院系名称不能为空");return}if($("#name").val().length>20){bootbox.alert("部门/院系名称不能超过20个字符");return}if($("#tel").val().length===0){bootbox.alert("联系电话不能为空");return}if($("#tel").val().length>20){bootbox.alert("联系电话不能超过20个字符");return}var n={};n.Id=$(this).attr("data-id");n.Name=$("#name").val();n.Address=$("#address").val();n.Tel=$("#tel").val();n.Email=$("#email").val();n.Weibo=$("#weibo").val();n.Wechat=$("#wechat").val();n.QQ=$("#qq").val();n.Introduction=$("#editor").val();n.IsEnabled=$("#enable").val();n.IsBranch=$("#branch").val();$(this).attr("disabled","disabled");$.ajax({type:"POST",url:"/Administrator/School/EditDepartment",data:n,dataType:"json",success:function(n){n.msg!==undefined&&(bootbox.dialog({message:n.msg,closeButton:!1}),n.success?setTimeout(function(){window.location.href="/Administrator/School/Department"},2e3):setTimeout(function(){window.location.href=window.location.href},2e3));$("#save").removeAttr("disabled")},error:function(n){console.log(n);$("#save").removeAttr("disabled")}})})});