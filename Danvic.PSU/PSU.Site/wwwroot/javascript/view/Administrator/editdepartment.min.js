 ...
 *   Author: Danvic712
 */
$(function(){var n=new Simditor({textarea:$("#editor")});$(document).on("click","#save",function(){if($("#name").length===0){bootbox.alert("院系名称不能为空");return}if($("#name").length>20){bootbox.alert("院系名称不能超过20个字符");return}if($("#tel").length===0){bootbox.alert("联系电话不能为空");return}if($("#tel").length>20){bootbox.alert("联系电话不能超过20个字符");return}var n={};n.Id=$(this).attr("data-id");n.Title=$("#title").val();n.Target=$("#target").val();n.Type=$("#type").val();n.Content=$("#editor").val();$(this).attr("disabled","disabled");$.ajax({type:"POST",url:"/Administrator/Home/Edit",data:n,dataType:"json",success:function(n){n.msg!==undefined&&(bootbox.dialog({message:n.msg,closeButton:!1}),n.success&&setTimeout(function(){window.location.href="/Administrator/Home/Bulletin"},2e3));$("#save").removeAttr("disabled")},error:function(n){console.log(n);$("#save").removeAttr("disabled")}})})});