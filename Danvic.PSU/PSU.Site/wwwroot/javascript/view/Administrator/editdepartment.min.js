 ...
 *   Author: Danvic712
 */
$(function(){var n=new Simditor({textarea:$("#editor"),toolbar:["title","bold","italic","fontScale","color","ol","ul","table","link","hr","indent","outdent","alignment",]});$(document).on("click","#save",function(){if($("#name").val().length===0)return bootbox.dialog({message:"部门/院系名称不能为空"}),!1;if($("#tel").val().length===0)return bootbox.dialog({message:"联系电话不能为空"}),!1;var n={};n.Id=$(this).attr("data-id");n.Name=$("#name").val();n.Address=$("#address").val();n.Tel=$("#tel").val();n.Email=$("#email").val();n.Weibo=$("#weibo").val();n.Wechat=$("#wechat").val();n.QQ=$("#qq").val();n.Introduction=$("#editor").val();n.IsEnabled=$("#enable").val();n.IsBranch=$("#branch").val();$(this).attr("disabled","disabled");$.ajax({type:"POST",url:"/Administrator/School/EditDepartment",data:n,dataType:"json",success:function(n){n.msg!==undefined&&(bootbox.dialog({message:n.msg}),n.success&&setTimeout(function(){window.location.href="/Administrator/School/Department"},2e3));$("#save").removeAttr("disabled")},error:function(n){console.log(n);$("#save").removeAttr("disabled")}})})});