/*!
 *   Administrator Region Page JavaScript v1.0.0
 *   Author: Danvic712
 */
...
$.dataTableSetting = {
    ...
    ajax: function (data, callback, settings) {
        ...
        $.ajax({
            type: "POST",
            url: "/Administrator/Basic/SearchRegion",
            cache: false,  //禁用缓存
            data: {
...
$(function () {
    var dataTable = $('#region-table').dataTable($.dataTableSetting);
    ...
    $(document).on('click',
        ...
        function () {
            ...
            dataTable = $('#region-table').dataTable($.dataTableSetting);
        });
    ...
    $(document).on('click',
        '#edit',
        function () {
            window.location.href = '/Administrator/Basic/EditRegion/' + $(this).attr('data-id');
        });
    ...
    $(document).on('click',
        ...
        function () {
            ...
            window.bootbox.confirm({
                message: '地区编号：<b class="text-red">' + id + '</b>，确定删除该条地区数据吗？',
                buttons: {
                    confirm: {
                        label: '确定',
                ...
                callback: function (result) {
                    if (result) {
                        $.ajax({
                            url: '/Administrator/Basic/DeleteRegion',
                            type: 'POST',
                            dataType: 'Json',
                            data: {
                                id: id
                            ...
                            success: function (result) {
                                window.bootbox.alert({
                                    ...
                                    callback: function () {
                                        window.location = "/Administrator/Basic/Region";
                                    }
                                });
                            },
                            error: function (msg) {
