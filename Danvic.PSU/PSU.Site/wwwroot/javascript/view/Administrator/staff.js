/*!
 *   Administrator Staff Page JavaScript v1.0.0
 *   Author: Danvic712
 */
...
$.dataTableSetting = {
    ...
    ajax: function (data, callback, settings) {
        ...
        $.ajax({
            type: "POST",
            url: "/Administrator/Basic/SearchStaff",
            cache: false,  //禁用缓存
            data: {
...
$(function () {
    var dataTable = $('#staff-table').dataTable($.dataTableSetting);
    ...
    $(document).on('click',
        ...
        function () {
            ...
            dataTable = $('#staff-table').dataTable($.dataTableSetting);
        });
    ...
    $(document).on('click',
        '#edit',
        function () {
            window.location.href = '/Administrator/Basic/EditStaff/' + $(this).attr('data-id');
        });
    ...
    $(document).on('click',
        ...
        function () {
            ...
            window.bootbox.confirm({
                message: '教职工工号：<b class="text-red">' + id + '</b>，确定删除该条教职工数据吗？',
                buttons: {
                    confirm: {
                        label: '确定',
                ...
                callback: function (result) {
                    if (result) {
                        $.ajax({
                            url: '/Administrator/Basic/DeleteStaff',
                            type: 'POST',
                            dataType: 'Json',
                            data: {
                                id: id
                            ...
                            success: function (result) {
                                window.bootbox.alert({
                                    ...
                                    callback: function () {
                                        window.location = "/Administrator/Basic/Staff";
                                    }
                                });
                            },
                            error: function (msg) {
