 ...
 *   Author: Danvic712
 */
$(function(){new AjaxUpload($("#iconpath"),{action:"/Administrator/Admission/Upload",onSubmit:function(n,t){if(!(t&&/^(GIF|BMP|PNG|JPG|JPEG)$/.test(t)))return bootbox.alert({message:"请上传指定类型的图片",size:"small"}),!1;var i=$(this)["0"]._input.files["0"].size;if(i>1048576)return bootbox.alert({message:"请上传指定大小文件",size:"small"}),!1},onComplete:function(n,t){t=$.parseJSON(t.replace(/<.*?>/ig,""));var i;t.success?(i=bootbox.dialog({message:'<p class="text-center">上传成功<\/p>',size:"small"}),setTimeout(function(){i.modal("hide")},1e3),$("#hideUrl").val(t.path),$("#pathName").html(n),$("#fname").html("图片限制1M以内，仅支持GIF,BMP,PNG,JPG,JPEG图片")):(i=bootbox.dialog({message:'<p class="text-center">'+t.msg+"<\/p>",size:"small"}),setTimeout(function(){i.modal("hide")},1e3))}});$(document).on("click","#save",function(){if($("#edit_name").val()==="")return bootbox.dialog({message:"物品名称不能为空"}),!1;var n={};n.Id=$(this).attr("data-id");n.Name=$("#edit_name").val();n.Description=$("#edit_description").val();n.Size=$("#edit_size").val();n.ImageSrc=$("#edit_hideUrl").val();n.IsEnabled=$("#edit_enable").val();$(this).attr("disabled","disabled");$.ajax({type:"POST",url:"/Administrator/Admission/EditGoods",data:n,dataType:"json",success:function(n){n.msg!==undefined&&(bootbox.dialog({message:n.msg}),n.success&&setTimeout(function(){window.location.href="/Administrator/Admission/Goods"},2e3));$("#save").removeAttr("disabled")},error:function(n){console.log(n);$("#save").removeAttr("disabled")}})})});