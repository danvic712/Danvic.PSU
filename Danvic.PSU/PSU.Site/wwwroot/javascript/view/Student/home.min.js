 ...
 *   Author: Danvic712
 */
$(function(){var n=new Simditor({textarea:$("#editor"),toolbar:["title","bold","italic","fontScale","color","ol","ul","table","link","hr","indent","outdent","alignment",]});$(document).on("click","#save",function(){if($("#editor").val()==="")return bootbox.dialog({message:"提问信息为空"}),!1;var n={};n.Content=$("#editor").val();$(this).attr("disabled","disabled");$.ajax({type:"POST",url:"/Student/Home/Ask",data:n,dataType:"json",success:function(n){n.msg!==undefined&&(bootbox.dialog({message:n.msg}),n.success&&setTimeout(function(){window.location.href="/Student"},2e3));$("#save").removeAttr("disabled")},error:function(n){console.log(n);$("#save").removeAttr("disabled")}})})});